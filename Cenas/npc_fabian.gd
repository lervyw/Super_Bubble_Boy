...
@onready var dialog_panel: Panel = $"../DialogLayer/DialogPanel"
@onready var name_label: Label = $"../DialogLayer/DialogPanel/NameLabel"
@onready var dialog_label: RichTextLabel = $"../DialogLayer/DialogPanel/DialogLabel"
@onready var continue_prompt: Button = $"../DialogLayer/DialogPanel/ContinuePrompt"
...
func _update_prompt_icon() -> void:
	if prompt_icon:
		prompt_icon.texture = PromptIcons.for_action(&"interact")
...
func _process(_delta: float):
	...
	if player:
		...
		if state == State.IDLE:
			...
			prompt_label.visible = true
			if Input.is_action_just_pressed("interact"):
				print("[Fabian] W — restartando diálogo")
				start_dialog()
			return
	else:
