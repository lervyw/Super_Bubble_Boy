...
const ENEMY_PLATFORM_COYOTE_TIME: float = 0.15
const LEDGE_COYOTE_TIME: float = 0.12
...
@export_range(0.1, 20.0, 0.1) var camera_offset_smoothing: float = 2.0
@export_range(0.1, 20.0, 0.1) var camera_position_smoothing_speed: float = 4.0
@export_range(0.0, 1.0, 0.01) var camera_drag_left_margin: float = 0.25
...
var on_ground := false
var enemy_platform_coyote_timer := 0.0
var ledge_coyote_timer := 0.0
...
func _physics_process(delta: float) -> void:
	...
	if is_grounded_on_enemy():
		enemy_platform_coyote_timer = ENEMY_PLATFORM_COYOTE_TIME
	else:
		enemy_platform_coyote_timer = max(enemy_platform_coyote_timer - delta, 0.0)

	if is_on_floor():
		ledge_coyote_timer = LEDGE_COYOTE_TIME
	else:
		ledge_coyote_timer = max(ledge_coyote_timer - delta, 0.0)

	on_ground = is_on_floor() or enemy_platform_coyote_timer > 0.0
	if on_ground:
		bubble_jump_count = 0
...
func is_grounded_on_enemy() -> bool:
	...
	if not stomper or not stomper.monitoring:
		return false
	for area in stomper.get_overlapping_areas():
		if resolve_stomp_target(area) != null:
			return true
	return false
	...
	match form:
		...
		Form.SUPER:
			if in_water:
				...
				ledge_coyote_timer = 0.0
			elif enemy_platform_coyote_timer > 0.0:
				velocity.y = super_jump_force
				enemy_platform_coyote_timer = 0.0
			elif ledge_coyote_timer > 0.0:
				velocity.y = super_jump_force
				ledge_coyote_timer = 0.0
		_:
			if in_water:
...
func jump_state() -> void:
	...
	if Input.is_action_just_pressed("attack") and not hud_menu_open and form in [Form.NORMAL, Form.SUPER]:
		start_normal_attack()
		return
...
func get_vertical_swim_axis() -> float:
	return Input.get_axis("swim_up", "crouch")


func is_swim_up_pressed() -> bool:
	return Input.is_action_pressed("swim_up")
...
func start_normal_attack() -> void:
	...
	if form == Form.NORMAL and stats:
		if not consume_stamina(stats.max_stamina * 0.5):
			return
...
func start_invincibility() -> void:
	...
	while elapsed < invincibility_time:
		...
		if sprite:
			sprite.visible = visible_on
		var wait_timer := tree.create_timer(HIT_FLASH_BLINK_INTERVAL)
		if wait_timer == null:
			break
		await wait_timer.timeout
		elapsed += HIT_FLASH_BLINK_INTERVAL
...
func update_camera_wall_limits() -> void:
	if not player_camera:
		return

	var space_state := get_world_2d().direct_space_state
	var lower_origin := global_position
	var upper_origin := global_position - Vector2(0, camera_wall_min_span)

	var right_wall_x: Variant = find_big_wall_x(space_state, lower_origin, upper_origin, 1.0)
	var left_wall_x: Variant = find_big_wall_x(space_state, lower_origin, upper_origin, -1.0)

	player_camera.limit_right = int(right_wall_x) if right_wall_x != null else CAMERA_LIMIT_UNSET
	player_camera.limit_left = int(left_wall_x) if left_wall_x != null else -CAMERA_LIMIT_UNSET


func find_big_wall_x(space_state: PhysicsDirectSpaceState2D, lower_origin: Vector2, upper_origin: Vector2, dir: float) -> Variant:
	var offset := Vector2(dir * camera_wall_check_distance, 0)

	var lower_query := PhysicsRayQueryParameters2D.create(lower_origin, lower_origin + offset)
	lower_query.collision_mask = CAMERA_WORLD_LAYER_MASK
	lower_query.exclude = [self]
	var lower_hit := space_state.intersect_ray(lower_query)
	if lower_hit.is_empty():
		return null

	var upper_query := PhysicsRayQueryParameters2D.create(upper_origin, upper_origin + offset)
	upper_query.collision_mask = CAMERA_WORLD_LAYER_MASK
	upper_query.exclude = [self]
	var upper_hit := space_state.intersect_ray(upper_query)
	if upper_hit.is_empty():
		return null

	return lower_hit.position.x - dir * camera_wall_margin
