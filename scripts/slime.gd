...
@export_range(16.0, 200.0, 1.0) var head_slide_vertical_range: float = 64.0
...
func avoid_nearby_slimes() -> void:
	...
	for other in get_tree().get_nodes_in_group("slime"):
		if other == self or not is_instance_valid(other):
			continue
		if "dying" in other and other.dying:
			continue
		var offset: Vector2 = global_position - other.global_position
		var dist := offset.length()
		if dist < 1.0 or dist > separation_distance:
			continue
		push += offset.normalized() * ((separation_distance - dist) / separation_distance)

	if push == Vector2.ZERO:
		return

	velocity += push.normalized() * separation_strength * get_water_speed_multiplier()
...
func separate_from_player():
	...
	var speed_scale := get_water_speed_multiplier()

	# Em cima do jogador: empurra para o lado sempre, independente do
	# jogador estar se movendo, para nunca grudar na cabeca. So conta como
	# "em cima" se estiver perto o suficiente na vertical tambem, senao
	# slimes aquaticos nadando bem acima do jogador seriam empurrados
	# mesmo sem chegar perto de encostar.
	# Usa atribuicao (nao soma) para o empurrao nao acumular frame a frame.
	# Escala pela velocidade na agua para nao virar um empurrao exagerado
	# nos slimes aquaticos, que nadam bem mais devagar que andam.
	if offset.y < 0 and offset.y > -head_slide_vertical_range and absf(offset.x) < stop_distance * 1.5:
		var side := -1.0 if offset.x < 0 else 1.0
	...
	velocity.x = push_dir.x * base_push * speed_scale
